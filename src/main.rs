...
fn main() {
    ...
    // Load GPU-accelerated Haar cascade
    let mut face_cascade = CascadeClassifier::create(cascade_path)
        .expect("Failed to load Haar cascade file. Ensure the path is correct and the file exists.");
    ...
    let stream = cuda::Stream::default().unwrap();
    ...
    while cap.read(&mut frame).unwrap() {
        ...
        // Upload frame to GPU
        let mut gpu_frame = cuda::GpuMat::default();
        gpu_frame.upload(&frame, &stream).unwrap();

        // Convert to grayscale for face detection
        let mut gray_gpu_frame = cuda::GpuMat::default();
        cuda::cvt_color(&gpu_frame, &mut gray_gpu_frame, imgproc::COLOR_BGR2GRAY, 0, &stream)
            .unwrap();

        // Detect faces using CUDA-accelerated Haar cascade
        let mut faces = Vector::<Rect>::new();
